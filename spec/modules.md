...
3. **`irq_ctrl`（中断控制，可并入 csr_if）**
   - `done_irq` / `err_irq` 生成
   - `irq_en` 屏蔽与中断状态锁存

4. **`tile_scheduler`（主调度器）**
   - 三重循环 `(m0,n0,k0)` 管理
...
6. **`rd_addr_gen`（读地址发生器）**
   - 按 `(m0,n0,k0)` + stride 计算 AXI 地址
   - 支持边界长度裁剪、burst 拆分

7. **`axi_rd_master`（AXI4 读通道）**
   - AR/R 通道协议
   - 突发读、beat 拼包、错误响应上报
...
9. **`wr_addr_gen`（写地址发生器）**
   - D tile 基址 + stride 映射
   - 边界写长度控制

10. **`axi_wr_master`（AXI4 写通道）**
   - AW/W/B 通道协议
   - burst 写、最后拍标记、响应检查

## 3) 片上缓存与数据重排模块

11. **`buffer_bank`（统一 buffer 管理）**
   - `A_BUF[2] / B_BUF[2] / C_BUF / (ACC_BUF)`
   - bank 冲突仲裁、读写端口分配
   - ping-pong 读写角色切换

12. **`a_loader`（A 数据装载/重排）**
   - DMA 读数据写入 A buffer
   - 按阵列喂数顺序做布局转换

13. **`b_loader`（B 数据装载/重排）**
   - DMA 读数据写入 B buffer
   - 按列方向输入顺序重排

14. **`c_loader`（C 数据装载，可选）**
...
21. **`fp_add_c`（与 C 融合单元）**
   - 可旁路（不使用 C 时）
   - 支持边界 mask

22. **`fp_round_sat`（舍入/饱和）**
